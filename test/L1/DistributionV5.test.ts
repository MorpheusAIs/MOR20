...
describe('DistributionV5', () => {
  ...
  let distributionFactory: DistributionToArbV5__factory;
  let distribution: DistributionToArbV5;
  let distributionBase: DistributionToBaseV5;
  ...
  before(async () => {
    ...
    distributionFactory = await ethers.getContractFactory('DistributionToArbV5', {
      libraries: {
    ...
    const distributionToBaseFactory = await ethers.getContractFactory('DistributionToBaseV5', {
      libraries: {
        LinearDistributionIntervalDecrease: await lib.getAddress(),
        ReferrerLib: await ReferrerLib.getAddress(),
      },
    ...
    distribution = distributionFactory.attach(await distributionProxy.getAddress()) as DistributionToArbV5;
    ...
    await distribution.DistributionToArbV5_init(depositToken, l1ArbSender, feeConfig, []);
    await distributionBase.DistributionToBaseV5_init(depositToken, l1BaseSender, feeConfig, []);
  ...
  describe('initialization', () => {
    describe('#constructor', () => {
      it('should disable initialize function', async () => {
        ...
        await expect(
          distribution.DistributionToArbV5_init(depositToken, l1ArbSender, feeConfig, []),
    ...
    describe('#DistributionToArb_init', () => {
      ...
      it('should create pools with correct data', async () => {
        ...
        const distribution = distributionFactory.attach(await distributionProxy.getAddress()) as DistributionToArbV5;

        await distribution.DistributionToArbV5_init(depositToken, l1ArbSender, feeConfig, [pool1, pool2]);
      ...
      it('should revert if try to call init function twice - Base', async () => {
        ...
        await expect(
          distributionBase.DistributionToBaseV5_init(depositToken, l1ArbSender, feeConfig, []),
        ).to.be.rejectedWith(reason);
      });
  ...
  describe('#claim', () => {
    ...
    it('should correctly claim, real data', async () => {
      ...
      await distribution.connect(COMMUNITY).stake(poolId + 1, wei(1), 0, ZERO_ADDR);
    ...
    describe('with referrer', () => {
      ...
      it('should correctly claim, few users, without redeposits', async () => {
        ...
        await distribution.connect(SECOND).claim(poolId, SECOND, { value: wei(0.5) });
        await distribution.claimReferrerTier(poolId, REFERRER_1, { value: wei(0.5) }); // Claim from OWNER to REFERRER_1
        // await ethers.provider.send('evm_setAutomine', [true]);
        // await ethers.provider.send('evm_mine');
        ...
        expect(await rewardToken.balanceOf(SECOND.address)).to.closeTo(secondAmount, wei(0.01));
        userData = await distribution.usersData(SECOND.address, poolId);
        expect(userData.deposited).to.eq(wei(1));
        expect(userData.virtualDeposited).to.eq(wei(1.01));
        expect(userData.pendingRewards).to.eq(0);
  ...
  describe('#bridgeOverplusToBase', () => {
    ...
    it('should bridge overplus with basic fee', async () => {
      ...
      await distributionBase.stake(1, wei(1), 0, ZERO_ADDR);
  ...
  describe('#version', () => {
    it('should return correct version', async () => {
      expect(await distributionBase.version()).to.eq(5);
    });
  });
});
...
const _getRewardTokenFromPool = async (
  distribution: DistributionToArbV5 | DistributionToBaseV5,
  amount: bigint,
...
const _getNextPoolId = async (distribution: DistributionToArbV5 | DistributionToBaseV5) => {
  let poolId = 0;
...
// npx hardhat test "test/L1/DistributionV5.test.ts"
// npx hardhat coverage --solcoverjs ./.solcover.ts --testfiles "test/L1/DistributionV5.test.ts"
