...
import {
  DistributionToArbV4,
  DistributionToArbV4__factory,
  DistributionToBaseV4,
  FeeConfig,
  GatewayRouterMock,
  IDistributionV4,
  IL1ArbSender,
...
describe('DistributionV4', () => {
  ...
  let distributionFactory: DistributionToArbV4__factory;
  let distribution: DistributionToArbV4;
  let distributionBase: DistributionToBaseV4;
  ...
  before(async () => {
    ...
    distributionFactory = await ethers.getContractFactory('DistributionToArbV4', {
      libraries: {
    ...
    const distributionToBaseFactory = await ethers.getContractFactory('DistributionToBaseV4', {
      libraries: {
        LinearDistributionIntervalDecrease: await lib.getAddress(),
      },
    });
    ...
    distribution = distributionFactory.attach(await distributionProxy.getAddress()) as DistributionToArbV4;
    ...
    await distribution.DistributionToArbV4_init(depositToken, l1ArbSender, feeConfig, []);
    await distributionBase.DistributionToBaseV4_init(depositToken, l1BaseSender, feeConfig, []);
  ...
  describe('initialization', () => {
    describe('#constructor', () => {
      it('should disable initialize function', async () => {
        ...
        await expect(
          distribution.DistributionToArbV4_init(depositToken, l1ArbSender, feeConfig, []),
    ...
    describe('#DistributionToArb_init', () => {
      ...
      it('should create pools with correct data', async () => {
        ...
        const distribution = distributionFactory.attach(await distributionProxy.getAddress()) as DistributionToArbV4;

        await distribution.DistributionToArbV4_init(depositToken, l1ArbSender, feeConfig, [pool1, pool2]);

        const pool1Data: IDistributionV4.PoolStruct = await distribution.pools(0);
        expect(_comparePoolStructs(pool1, pool1Data)).to.be.true;

        const pool2Data: IDistributionV4.PoolStruct = await distribution.pools(1);
      ...
      it('should revert if try to call init function twice - Base', async () => {
        ...
        await expect(
          distributionBase.DistributionToBaseV4_init(depositToken, l1ArbSender, feeConfig, []),
        ).to.be.rejectedWith(reason);
      });
  ...
  describe('#createPool', () => {
    it('should create pool with correct data', async () => {
      ...
      const poolData: IDistributionV4.PoolStruct = await distribution.pools(0);
  ...
  describe('#withdraw', () => {
    ...
    it('should correctly withdraw, when not enough tokens', async () => {
      await distribution.stake(poolId, wei(0.1), 0);
      await distribution.connect(SECOND).stake(poolId, wei(0.1), 0);
  ...
  describe('#getCurrentUserReward', () => {
    ...
    it('should correctly calculate distribution rewards with real data', async () => {
      const pool: IDistributionV4.PoolStruct = {
        payoutStart: oneDay,
  ...
  describe('#version', () => {
    it('should return correct version', async () => {
      expect(await distributionBase.version()).to.eq(4);
    });
  });
});
...
const _getRewardTokenFromPool = async (
  distribution: DistributionToArbV4 | DistributionToBaseV4,
  amount: bigint,
...
const _getNextPoolId = async (distribution: DistributionToArbV4 | DistributionToBaseV4) => {
  let poolId = 0;
...
const _comparePoolStructs = (a: IDistributionV4.PoolStruct, b: IDistributionV4.PoolStruct): boolean => {
  return (
...
// npx hardhat test "test/L1/DistributionV4.test.ts"
// npx hardhat coverage --solcoverjs ./.solcover.ts --testfiles "test/L1/DistributionV4.test.ts"
